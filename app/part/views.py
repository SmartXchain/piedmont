...
def jobdetails_add_view(request, part_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Assign the part_detail (you may refine the logic to pick specific PartDetails)
            job.part_detail = PartDetails.objects.filter(part=part).first()
            if not job.part_detail:
                form.add_error(None, "No associated PartDetails found for this part. Please create PartDetails first.")
            else:
                job.save()
                return redirect('part_detail', part_id=part.id)  # Redirect to part detail page
    else:
        form = JobDetailsForm()

    return render(request, 'part/jobdetails_form.html', {'form': form, 'part': part, 'job': None})
