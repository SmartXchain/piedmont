...
- Added: Expanded process app test coverage to 27 tests — `TestProcessStr` (2), `TestProcessStepStr` (1), `TestProcessLandingView` (6), `TestProcessFlowchartView` (4), `TestProcessFlowchartDownloadView` (4), `TestGetClassificationsView` (3), `TestGetMethodInfoView` (4); covers model `__str__`, all views, both AJAX endpoints, auth redirects, search, and 404 handling; flowchart views mock Graphviz for speed (PR-8)
- Changed: Applied TAT color scheme to `process_landing.html` — TAT dark `thead`, blue Search/View Flowchart buttons, dashed empty state, blue underline heading accent; breadcrumb Home → Processes added (PR-6, PR-7)
- Changed: Applied TAT color scheme to `process_flowchart.html` — TAT dark card header, TAT blue outline Download SVG button; breadcrumb Home → Processes → [Standard] Flowchart added; Back button replaced `javascript:history.back()` with explicit `process_landing` URL (PR-6, PR-7)
- Changed: Replaced `fields = '__all__'` with explicit field list in `ProcessForm` — excludes auto-managed `created_at`/`updated_at` fields (PR-4)
- Removed: Dead `step_count_display()` method from `process/admin.py` — never added to `list_display`, had an N+1 query per row and referenced a non-existent `_step_count` ordering annotation (PR-3)
- Removed: No-op `ProcessStep.save()` override in `process/models.py` — only called `super().save()` with no custom logic (PR-2)
- Fixed: Replaced `Method.objects.get()` + bare `except` with `get_object_or_404()` in `process/views.py:get_method_info()` (PR-1)
- Added: Expanded standard app test coverage — 41 tests total across `tests.py` and `tests/test_views.py`; rewrote view tests with authentication, added `TestStandardListView` (8), `TestStandardDetailView` (6), `TestProcessReviewView` (6) covering 200/404/auth/context/POST/redirect; added `TestStandardProcessStr`, `TestClassificationStr`, `TestStandardRevisionNotificationStr` model string tests (ST-6)
- Changed: Replaced `unique_together` with `UniqueConstraint` in `StandardProcess` (`unique_standard_process_title`) and `StandardPeriodicRequirement` (`unique_standard_periodic_requirement`); migration `standard/0023_...` generated and applied (ST-2)
- Removed: Unused imports `Q` from `standard/models.py`, `Count` from `standard/views.py`, `redirect` from `standard/admin.py` (ST-3)
- Removed: Dead utility functions `create_standard()`, `list_standards()`, `get_standard_by_id()` from `standard/models.py` — never called anywhere in the codebase (ST-1)
- Fixed: Removed `test_standard_create_view` from `standard/tests/test_views.py` — referenced `standard_create` URL that does not exist, causing `NoReverseMatch` on every test run; standards are managed through Django admin (ST-5)
- Changed: Applied TAT Technologies color scheme to all standard app templates — dark card headers (`var(--tat-dark)`), blue badges, TAT blue action buttons, TAT dark table headers, active accordion items in TAT dark; moved inline scripts to `{% block extra_js %}` (ST-7)
- Fixed: Corrected breadcrumbs on all standard app templates — `standard_list.html` first crumb now links to `{% url 'home' %}` (was linking to `standard_list`); `standard_detail.html` now has full Home → Standards → [Standard] chain; `process_review.html` has correct chain from creation (ST-8)
- Fixed: Created missing `standard/process_review.html` template — view existed but template did not, causing TemplateDoesNotExist on any visit to `/standards/process-review/`; template lists pending standards with acknowledge form, TAT color scheme, breadcrumb, empty state, and Django messages support (ST-4)
